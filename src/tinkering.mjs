"use strict"; export class Tinkering extends ArtisanSkill {
    constructor(namespace, game) { super(namespace, 'Tinkering', game); this._media = 'assets/temp.png'; this.baseInterval = 2000; this.selectionTabs = smithingSelectionTabs; this.renderQueue = new ArtisanSkillRenderQueue(); this.oreToBarMap = new Map(); this.categories = new NamespaceRegistry(game.registeredNamespaces); }
    getTotalUnlockedMasteryActions() { return this.actions.reduce(levelUnlockSum(this), 0); }
    get menu() { return smithingArtisanMenu; }
    get noCostsMessage() { return getLangString('TOASTS_MATERIALS_REQUIRED_TO_SMITH'); }
    ...
    getUncappedDoublingChance(action) {
        let chance = super.getUncappedDoublingChance(action); const masteryLevel = this.getMasteryLevel(action); chance += Math.floor((masteryLevel + 10) / 20) * 5; if (masteryLevel >= 99)
            chance += 10; if (this.isPoolTierActive(3))
            chance += 10; return chance;
    }
    getPreservationChance(action, chance) {
        const masteryLevel = this.getMasteryLevel(action); chance += Math.floor(masteryLevel / 20) * 5; if (masteryLevel >= 99)
    ...
    getMasteryXPModifier(action) {
        let modifier = super.getMasteryXPModifier(action); if (this.isPoolTierActive(0))
            modifier += 5; return modifier;
    }
    onMasteryLevelUp(action, oldLevel, newLevel) {
        super.onMasteryLevelUp(action, oldLevel, newLevel); if (this.selectedRecipe === action)
            this.renderQueue.selectedRecipe = true;
    }
    modifyItemCost(item, quantity, recipe) {
        if (item.id === "melvorD:Coal_Ore") { quantity = applyModifier(quantity, this.game.modifiers.decreasedSmithingCoalCost, 2); quantity -= this.game.modifiers.decreasedFlatSmithingCoalCost; return quantity; }
    ...
    preAction() { }
    get actionRewards() {
        const rewards = new Rewards(this.game); const actionEvent = new SmithingActionEvent(this, this.activeRecipe); let qtyToAdd = this.actionItemQuantity; if (rollPercentage(this.game.modifiers.summoningSynergy_3_17))
            qtyToAdd++; if (rollPercentage(this.actionDoublingChance))
            qtyToAdd *= 2; qtyToAdd *= Math.pow(2, this.game.modifiers.getSkillModifierValue('doubleItemsSkill', this)); const item = this.actionItem; if (item.id === "melvorD:Silver_Bar") {
                let goldBarQty = 1; const activePotion = this.activePotion; if (this.game.modifiers.doubleSilverGoldSmithingWithSeeingGold > 0 && activePotion !== undefined && ["melvorF:Seeing_Gold_Potion_I", "melvorF:Seeing_Gold_Potion_II", "melvorF:Seeing_Gold_Potion_III", "melvorF:Seeing_Gold_Potion_IV",].includes(activePotion.id)) { goldBarQty *= 2; qtyToAdd *= 2; }
